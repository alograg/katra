...
function adapterAction(&$amfbody)
{
    ...
    if (strpos($target, "http://") === FALSE
            && strpos($target, "https://") === FALSE) { // check for a http link which means web service
        $lpos = strrpos($target, ".");
        if ($lpos === FALSE) {
            ...
            if ($messageType == 'flex.messaging.messages.RemotingMessage') {
                ...
                $uriclasspath = str_replace('.', '/', $classAndPackage)
                        . '.php';
            ...
            if ($lpos === FALSE) {
                ...
                if ($classname == "PageAbleResult"
                    ...
                    $classname = substr(strrchr('/' . $set['class'], '/'), 1,
                            -4);
                    //Now set args for body
                    $amfbody
                            ->setValue(
                                    array_merge($set['args'],
                                            array(
                                                    $val[1],
                                                    $val[2]
                                            )));
                ...
                } else if ($classname == "PageAbleResult"
                        && $methodname == 'release') {
                ...
                $classpath = $baseClassPath . str_replace(".", "/", $trunced)
                        . ".php"; // removed to strip the basecp out of the equation here
        ...
        trigger_error("Web services are not supported in this release",
                E_USER_ERROR);
    }
...
function executionAction(&$amfbody)
{
    ...
    if (!$amfbody->isSpecialHandling()
        ...
        if ($specialHandling == 'describeService') {
            ...
            $dataset = Executive::doMethodCall($amfbody, $construct, $method,
                    $args);
            $results = array(
                    "cursor" => $args[count($args) - 2] + 1,
                    "data" => $dataset
            );
            ...
            $results = Executive::doMethodCall($amfbody, $construct, $method,
                    $args); // do the magic
            global $amfphp;
        ...
        if ($results !== '__amfphp_error') {
            if ($specialHandling == 'RemotingMessage') {
                $wrapper = new AcknowledgeMessage(
                        $amfbody->getMetadata("messageId"),
