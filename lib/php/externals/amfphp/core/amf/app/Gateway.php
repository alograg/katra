...
class Gateway
{
    ...
    function service()
    {
        ...
        if (isset($GLOBALS["HTTP_RAW_POST_DATA"])
            ...
            if ($this->incomingMessagesFolder != NULL) {
                ...
                $pieces = explode(' ', $mt);
                file_put_contents(
                        $this->incomingMessagesFolder . 'in.' . $pieces[1]
                                . '.' . substr($pieces[0], 2) . ".amf",
                        $GLOBALS["HTTP_RAW_POST_DATA"]);
            }
            ...
            if (Headers::getHeader('serviceBrowser') == TRUE) {
                ...
                $time = (int) ((microtime_float()
                        - $GLOBALS['amfphp']['startTime']) * 1000);
                ...
                $output = substr($output, 0, $toAddPos) . $r
                        . substr($output, $toAddPos + 8);
            }
            ...
            if ($this->outgoingMessagesFolder != NULL) {
                ...
                $pieces = explode(' ', $mt);
                file_put_contents(
                        $this->outgoingMessagesFolder . 'out.' . $pieces[1]
                                . '.' . substr($pieces[0], 2) . ".amf",
                        $output);
            }
            $doCompress = FALSE;
            ...
            if (!$outputCompression) {
                if (strlen($output) > $this->_gzipCompressionThreshold
                        ...
                        && strpos($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip')
                                !== FALSE && $this->_enableGzipCompression) {
                    $doCompress = TRUE;
                    ob_start();
            ...
            if (function_exists("amf_decode")) {
                echo ("<p>AMF C Extension is loaded "
                        . ($GLOBALS['amfphp']['native'] ? "and enabled."
                                : "but disabled") . "</p>");
            }
    ...
    function disableStandalonePlayer($value = TRUE)
    {
        if ($value && $_SERVER['HTTP_USER_AGENT'] == "Shockwave Flash") {
            trigger_error(
                    "Standalone Flash player disabled. Update gateway.php to allow these connections",
                    E_USER_ERROR);
            die();
        }
    }
