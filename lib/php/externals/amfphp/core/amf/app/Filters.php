...
function authenticationFilter(&$amf)
{
    ...
    if ($authHeader !== FALSE
            && $authHeader->value !== AMFPHP_CLEARED_CREDENTIALS) {
        ...
        $outHeader = new MessageHeader("RequestPersistentHeader", TRUE,
                $clearHeader);
    ...
    if (!strpos($_SERVER['QUERY_STRING'], $session_id) !== FALSE) {
        ...
        $outHeader = new MessageHeader("AppendToGatewayUrl", FALSE,
                $joint . $sessionName . "=" . $session_id);
        $amf->addOutgoingHeader($outHeader);
    }
}
...
function batchProcessFilter(&$amf)
{
    ...
    for ($i = 0; $i < $bodycount; $i++) {
        ...
        foreach ($actions as $key => $action) {
            $results = call_user_func($action, $bodyObj);
            if ($results === FALSE) {
                break;
            }
        }
    ...
    for ($i = 0; $i < $bodycount; $i++) {
        $bodyObj = &$amf->getBodyAt($i);
        if ($bodyObj->getSpecialHandling() == 'auth'
                && $bodyObj->getResults() === NULL) {
            $amf->removeBodyAt($i);
            break;
        }
    }
...
function debugFilter(&$amf)
{
    ...
    if (!$GLOBALS['amfphp']['isFlashComm']
            && !$GLOBALS['amfphp']['disableTrace']) {
