...
class AMFDeserializer extends AMFBaseDeserializer
{
    ...
    {
        if ($this->native)
            return amfphp_decode($this->raw_data, $this->decodeFlags,
                    $this->current_byte,
                    array(
                            &$this,
                            "decodeCallback"
    ...
    function readAmf3Data()
    {
        ...
        switch ($type) {
            ...
            default:
                trigger_error("undefined Amf3 type encountered: " . $type,
                        E_USER_ERROR);
        }
    }
    ...
    function readAmf3Date()
    {
        ...
        if (($dateref & 0x01) == 0) {
            ...
            if ($dateref >= count($this->storedObjects)) {
                trigger_error('Undefined date reference: ' . $dateref,
                        E_USER_ERROR);
                return FALSE;
            }
    ...
    function readAmf3String()
    {
        ...
        if (($strref & 0x01) == 0) {
            ...
            if ($strref >= count($this->storedStrings)) {
                trigger_error('Undefined string reference: ' . $strref,
                        E_USER_ERROR);
                return FALSE;
            }
    ...
    function readAmf3Object()
    {
        ...
        if ($inline) {
            ...
            if ($inlineClassDef) {
                ...
                $typedObject = !is_null($typeIdentifier)
                        && $typeIdentifier != "";
