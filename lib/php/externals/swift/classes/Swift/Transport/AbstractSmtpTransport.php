...
abstract class Swift_Transport_AbstractSmtpTransport implements Swift_Transport
{
    ...
    public function start()
    {
        if (!$this->_started) {
            ...
            try {
                $this->_buffer
                        ->initialize($this->_getBufferParams());
    ...
    public function send(Swift_Mime_Message $message, &$failedRecipients = NULL)
    {
        ...
        try {
            $sent += $this->_sendTo($message, $reversePath, $to,
                            $failedRecipients);
            $sent += $this->_sendCc($message, $reversePath, $cc,
                            $failedRecipients);
            $sent += $this->_sendBcc($message, $reversePath, $bcc,
                            $failedRecipients);
        } catch (Exception $e) {
            $message->setBcc($bcc);
            throw $e;
    ...
    {
        $this->_assertResponseCode($this->_getFullResponse(0),
                        array(
                                220
                        ));
    }
            ...
            array $to, array &$failedRecipients)
    {
        ...
        return $this->_doMailTransaction($message, $reversePath,
                        array_keys($to), $failedRecipients);
    }
            ...
            array $cc, array &$failedRecipients)
    {
        ...
        return $this->_doMailTransaction($message, $reversePath,
                        array_keys($cc), $failedRecipients);
    }
            ...
            array $bcc, array &$failedRecipients)
    {
        ...
        foreach ($bcc as $forwardPath => $name) {
            ...
            $sent += $this->_doMailTransaction($message, $reversePath,
                            array(
                                    $forwardPath
                            ), $failedRecipients);
        }
