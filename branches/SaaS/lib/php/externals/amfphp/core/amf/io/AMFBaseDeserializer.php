...
class AMFBaseDeserializer
{
    ...
    function decodeCallback($event, $arg)
    {
        if ($event == 1) {
            ...
            //Serializable post-decode
            if ($type == 'flex.messaging.io.ArrayCollection'
                    || $type == 'flex.messaging.io.ObjectProxy') {
                return;
            } else {
                trigger_error(
    ...
    function readHeader()
    {
        ...
        while ($this->header_count--) { // loop over all of the header elements
            ...
            if ($this->native) {
                $content = amf_decode($this->raw_data, $this->decodeFlags,
                        $this->current_byte,
                        array(
                                &$this,
                                "decodeCallback"
            ...
            $this->amfdata
                    ->addHeader(new MessageHeader($name, $required, $content)); // save the name/value into the headers array
        }
    ...
    function readBody()
    {
        ...
        while ($this->body_count--) { // loop over all of the body elements
            ...
            if ($this->native)
                $data = amf_decode($this->raw_data, $this->decodeFlags,
                        $this->current_byte,
                        array(
                                &$this,
                                "decodeCallback"
    ...
    function mapClass($typeIdentifier)
    {
        ...
        $clazz = NULL;
        if(class_exists($typeIdentifier, TRUE)){
        ...
        if (isset($GLOBALS['amfphp']['incomingClassMappings'][$typeIdentifier])) {
            $mappedClass = str_replace('.', '/',
                    $GLOBALS['amfphp']['incomingClassMappings'][$typeIdentifier]);
        }

        $include = FALSE;
        if (file_exists(
                $GLOBALS['amfphp']['customMappingsPath'] . $mappedClass
                        . '.php')) {
            $include = $GLOBALS['amfphp']['customMappingsPath'] . $mappedClass
                    . '.php';
        } elseif (file_exists(
                $GLOBALS['amfphp']['customMappingsPath'] . $mappedClass
                        . '.class.php')) {
