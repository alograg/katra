...
class AMFBaseSerializer
{
    ...
    function AMFBaseSerializer()
    {
        ...
        $this->native = false; /*$GLOBALS['amfphp']['native']
                && function_exists('amf_decode');*/
    ...
    function serialize(&$amfout)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $this->outBuffer = "";
            if ($this->native)
                $this->outBuffer .= amf_encode($header->value,
                        $this->encodeFlags, $encodeCallback);
            else
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($this->native)
                $this->outBuffer .= amf_encode($body->getResults(),
                        $this->encodeFlags, $encodeCallback);
            else
    ...
    function encodeCallback($value)
    {
        ...
        if (is_object($value)) {
            ...
            if (AMFPHP_PHP5 && $className == 'domdocument') {
                return array(
                        $this->cleanXml($value->saveXml()),
                        1
                );
            } else if (array_key_exists($className,
                    $GLOBALS['amfphp']['adapterMappings'])) {
                $subtype = $GLOBALS['amfphp']['adapterMappings'][strtolower(
                        $className)];

                $classname = $subtype . "Adapter"; // full class name
                $includeFile = include_once(AMFPHP_BASE . "shared/adapters/"
                        . $classname . ".php"); // try to load the recordset library from the sql folder
                if (!$includeFile) {
                    trigger_error(
                            "The recordset filter class " . $classname
                                    . " was not found", E_USER_ERROR);
                }
                ...
                return array(
                        $this->cleanXml($value->dump_mem()),
                        1
                );
            } else if ($className == 'simplexmlelement') {
                return array(
                        $this->cleanXml($value->asXML()),
                        1
                );
    ...
    function getClassName(&$d)
    {
        ...
        if (isset(
                $GLOBALS['amfphp']['outgoingClassMappings'][strtolower(
                        $classname)])) {
            return $GLOBALS['amfphp']['outgoingClassMappings'][strtolower(
                    $classname)];
        }
        ...
        if (class_exists("ReflectionClass")) //Another way of doing things, by Renaun Erickson
 {
            ...
            $fullClassName = substr($fullClassName, 0,
                    strlen($fullClassName) - 4);
            $fullClassName = str_replace(DIRECTORY_SEPARATOR, '.',
                    $fullClassName);
