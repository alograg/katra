...
class AMFSerializer extends AMFBaseSerializer
{
    ...
    function writeReferenceIfExists($d)
    {
        ...
        if (($key = patched_array_search($d, $this->amf0StoredObjects, TRUE))
                !== FALSE) {
 ...
 elseif ((is_a($d, 'ArrayAccess') || is_a($d, 'ArrayObject'))
    ...
    function writeAmf3Int($d)
    {
        ...
        if ($d < 0x80) {
            ...
            $this->outBuffer .= chr($d >> 14 & 0x7f | 0x80)
                    . chr($d >> 7 & 0x7f | 0x80) . chr($d & 0x7f);
        } else {
            $this->outBuffer .= chr($d >> 22 & 0x7f | 0x80)
                    . chr($d >> 15 & 0x7f | 0x80) . chr($d >> 8 & 0x7f | 0x80)
                    . chr($d & 0xff);
        }
    }
    ...
    function getAmf3Int($d)
    {
        ...
        if ($d < 0x80) {
            ...
            return chr($d >> 14 & 0x7f | 0x80) . chr($d >> 7 & 0x7f | 0x80)
                    . chr($d & 0x7f);
        } else {
        ...
        if ($d >= -268435456 && $d <= 268435455)//check valid range for 29bits
 {
    ...
    {
        if (($key = patched_array_search($d, $this->storedObjects, TRUE))
                === FALSE && $key === FALSE) {
            if (count($this->storedObjects) < MAX_STORED_OBJECTS) {
                $this->storedObjects[] = &$d;
            }
