...
/**
 * The Executive class is responsible for executing the remote service method and
 * returning it's value.
 *
 * Currently the executive class is a complicated chain of filtering events
 * testing for various cases and
 * handling them.  Future versions of this class will probably be broken up into
 * many helper classes which will
 * use a delegation or chaining pattern to make adding new exceptions or handlers
 * more modular.  This will
 * become even more important if developers need to make their own custom header
 * handlers.
 *
 ...
 * @author Justin Watkins Gateway architecture, class structure, datatype io
 * additions
...
class Executive
{
    ...
    {
        try {
            $output = Executive::deferredMethodCall($bodyObj, $object, $method,
                    $args);
        } catch (Exception $fault) {
            if (get_class($fault) == "VerboseException") {
                $ex = new MessageException($fault->code, $fault->getMessage(),
                        $fault->file, $fault->line, 'AMFPHP_RUNTIME_ERROR');
            } else {
                $code = "AMFPHP_RUNTIME_ERROR";
                if ($fault->getCode() != 0) {
                    $code = $fault->getCode();
                }
                $ex = new MessageException(E_USER_ERROR, $fault->getMessage(),
                        $fault->getFile(), $fault->getLine(), $code);
            }
    ...
    function buildClass(&$bodyObj, $className)
    {
        ...
        try {
            ...
            //When constructing a class, getLine and getFile don't refer to the appropriate
            // thing,
            //hence this hack
            $ex = new MessageException(E_USER_ERROR, $fault->getMessage(),
                    $bodyObj->classPath, 'Undetermined line  in constructor',
                    'AMFPHP_BUILD_ERROR');
            MessageException::throwException($bodyObj, $ex);
     ...
     * an error handler which seems to break the convential rule for working with
     * exceptions
    ...
    function includeClass(&$bodyObj, $location)
    {
        ...
        try {
            ...
            if (get_class($fault) == "VerboseException") {
                $ex = new MessageException($fault->code, $fault->getMessage(),
                        $fault->file, $fault->line, 'AMFPHP_INCLUDE_ERROR');
            } else {
                $ex = new MessageException(E_USER_ERROR, $fault->getMessage(),
                        $fault->getFile(), $fault->getLine(),
                        'AMFPHP_INCLUDE_ERROR');
            }
            MessageException::throwException($bodyObj, $ex);
        }
        return $included;
